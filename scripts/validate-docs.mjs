...
// 截图必须在 aicue.top 实拍，尚未补齐前只校验已登记的引用，不强制每页配图。
// assets/screenshots/ 一旦建立，配图数量与图注规则自动恢复为强制。
const hasScreenshotAssets = fs.existsSync(path.join(root, 'assets/screenshots'))
...
for (const file of pages) {
  ...
  if (!Array.isArray(meta.screenshots)) {
    ...
    if (hasScreenshotAssets) errors.push(`${relative}: screenshots 必须至少登记一张与页面主题相关的图片`)
  } else {
  ...
  if (bodyImages.length === 0) {
    if (hasScreenshotAssets) errors.push(`${relative}: 正文必须显示至少一张与页面主题相关的图片`)
  ...
  if (relative.startsWith('guide/') && hasScreenshotAssets) {
